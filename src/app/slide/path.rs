...
pub fn slide_shape_q(mut path: &mut Vec<Vec2>, note: &Note, seg:&SlideSegment, outer_r:f32, spawn_cx:Vec2, pad: &PadGeom,
...
pub fn slide_shape_p(mut path: &mut Vec<Vec2>, note: &Note, seg:&SlideSegment, outer_r:f32, spawn_cx:Vec2, pad: &PadGeom,
                     svg: &PadSvgDef, scale: f32) {
    let get_zone_pos=|zone: PadZone|{
        let idx = (zone.to_id() - 1) as f32;
        let ang = -std::f32::consts::FRAC_PI_2 + PAD_ROTATION_RAD + idx * std::f32::consts::TAU / 8.0;
...
pub fn slide_shape_line(mut path: &mut Vec<Vec2>, note: &Note, seg: &SlideSegment, outer_r: f32, spawn_cx: Vec2, pad: &PadGeom, svg: &PadSvgDef, scale: f32) {
