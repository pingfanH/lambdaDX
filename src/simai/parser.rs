...
// Vendored from `maisimai` (MIT); keep its sources as-is rather than fixing
// upstream lints.
#![allow(unreachable_patterns, unused_mut, unused_variables, dead_code)]

use super::model::{Bpm, SimaiChart, SimaiFile, SimaiNote, SlidePattern};
...
pub fn parse_chart_text(text: &str) -> Result<SimaiChart, ParseError> {
    ...
    let mut hi_speed: f32 = 1.0; // Note-speed multiplier (`[x]` markers).
    ...
    for frag in cleaned.split(',') {
        ...
        parse_fragment(frag, measure, &mut divisor, &mut hi_speed, &mut chart)?;
        measure += 1.0 / divisor;
    }
    ...
    while i < bytes.len() {
        ...
        match c {
            ...
            '(' => {
                ...
                let bpm: f32 = inner.parse().map_err(|_| ParseError {
                    message: format!("invalid bpm: {inner}"),
                    offset: Some(i),
                })?;
                if bpm <= 0.0 {
                ...
                chart
                    .bpms
                    .retain(|b| (b.measure - base_measure).abs() > 0.0001);
                chart.bpms.push(Bpm {
            ...
            'A' | 'B' | 'C' | 'D' | 'E' => {
                ...
                pending_pseudo = false;
                parse_touch_note(bytes, &mut i, base_measure + m_offset, chart, *hi_speed)?;
            }
            ' ' | '\t' | '\n' | '\r' => {
                i += 1;
            }
            '[' => {
                // Hi-speed marker `[x]` (a bare number). Durations `[a:b]`,
                // `[bpm#a:b]` and `[a##b]` are consumed by the note parser, so
                // any `:`/`#` reaching here is an orphaned duration.
                let end = find_close(bytes, i, b']')?;
                let inner = std::str::from_utf8(&bytes[i + 1..end]).unwrap_or("");
                if inner.contains(':') || inner.contains('#') {
                    return Err(ParseError {
                        message: format!("unexpected duration `[{inner}]` without a note"),
                        offset: Some(i),
                    });
                }
                let hs: f32 = inner.parse().map_err(|_| ParseError {
                    message: format!("invalid hi-speed: {inner}"),
                    offset: Some(i),
                })?;
                if hs <= 0.0 {
                    return err(format!("non-positive hi-speed: {hs}"));
                }
                *hi_speed = hs;
                i = end + 1;
            }
...
struct DurationSpec {
    equivalent_bpm: Option<f32>,
    duration: f32,
    delay: Option<f32>,
    seconds: bool,
}

/// Parse a duration block. Fractional forms are measure-relative; `#X` and
/// `X##Y` forms are expressed in seconds by Simai.
fn parse_duration(bytes: &[u8], i: &mut usize) -> Result<DurationSpec, ParseError> {
    ...
    *i = end + 1;
    ...
    let den: f32 = den_str.parse().map_err(|_| ParseError {
        message: format!("invalid denominator: {den_str}"),
        offset: None,
    })?;
    let num: f32 = num_str.parse().map_err(|_| ParseError {
        message: format!("invalid numerator: {num_str}"),
        offset: None,
    })?;
    if den <= 0.0 {
        return Ok(DurationSpec {
            equivalent_bpm: eq_bpm,
            duration: 0.0,
            delay: None,
            seconds: false,
        });
...
fn parse_button_note(
    ...
    *i += 1;
    ...
    if is_hold {
        ...
        if *i < bytes.len() && bytes[*i] == b'[' {
            let spec = parse_duration(bytes, i)?;
            let cur_bpm = current_bpm_at(chart, measure);
            duration = duration_to_measures(&spec, cur_bpm).1;
        }
        chart.notes.push(SimaiNote::Hold {
            measure,
            button,
            duration,
            is_ex,
    ...
    *i = end_idx;
    ...
    loop {
        ...
        if bytes[*i] == b'[' {
            ...
            let _ = parse_duration(bytes, i)?;
            let has_chain =
                *i < bytes.len() && (bytes[*i] == b'*' || is_slide_pattern_char(bytes[*i] as char));
            if !has_chain {
                *i = saved_i;
                break;
            }
    ...
    let mut shared_dur: Option<DurationSpec> = None;
    if *i < bytes.len() && bytes[*i] == b'[' {
        shared_dur = Some(parse_duration(bytes, i)?);
    }
    let cur_bpm = current_bpm_at(chart, measure);
    // The slide motion starts at the note head (delay 0) unless the chart
    // explicitly specifies one with the `[delay##duration]` seconds form.
    let (delay, dur) = shared_dur
        .as_ref()
        .map(|spec| duration_to_measures(spec, cur_bpm))
        .unwrap_or((0.0, 0.0));
    // Only the `[delay##duration]` form carries an explicit delay; a bare
    // `[0##x]` therefore means "no wait", not "use the default wait".
    let delay_explicit = shared_dur.as_ref().is_some_and(|spec| spec.delay.is_some());
...
fn duration_to_measures(spec: &DurationSpec, cur_bpm: f32) -> (f32, f32) {
    let seconds_to_measures = cur_bpm / 240.0;
    if spec.seconds {
        return (
            spec.delay.unwrap_or(0.0) * seconds_to_measures,
            spec.duration * seconds_to_measures,
        );
    }

    // Fraction `[D:N]` and equivalent-BPM `[eq#D:N]` forms express the slide
    // travel time only; the motion starts at the head, so the delay is 0.
    // `eq` rescales the travel time to the current BPM.
    if let Some(eq) = spec.equivalent_bpm {
        if eq > 0.0 {
            let mult = cur_bpm / eq;
            return (0.0, spec.duration * mult);
        }
    }
    (0.0, spec.duration)
}
...
fn read_slide_arc(bytes: &[u8], mut i: usize) -> Result<(SlideArc, usize), ParseError> {
    ...
    let (pattern, advance) = match c {
        ...
        other => {
            return Err(ParseError {
                message: format!("unknown slide pattern `{other}`"),
                offset: Some(i),
            })
        }
    };
...
fn parse_touch_note(
    ...
    *i += 1;
    ...
    if is_hold {
        ...
        if *i < bytes.len() && bytes[*i] == b'[' {
            let spec = parse_duration(bytes, i)?;
            duration = duration_to_measures(&spec, current_bpm_at(chart, measure)).1;
        }
        chart.notes.push(SimaiNote::TouchHold {
            measure,
            region,
            position,
            duration,
        ...
        chart.notes.push(SimaiNote::TouchTap {
            measure,
            region,
            position,
            is_firework,
