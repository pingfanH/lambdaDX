...
// ── Blender color palette (mirrors editor) ───────────────────────────
...
fn apply_blender_style(ctx: &egui::Context) {
    ...
    visuals.window_fill = Color32::from_rgb(45, 45, 45);
...
fn import_from_path(app: &mut PlayerState) {
    ...
    TopBottomPanel::top("toolbar")
        ...
        .show(ctx, |ui| {
            ui.horizontal_centered(|ui| {
                // ── App title ──
                ui.label(egui::RichText::new("LambdaDX Player")
                ...
                if toggle_btn(ui, "Replay", false) { app.toggle_replay(); }
                section_sep(ui);
